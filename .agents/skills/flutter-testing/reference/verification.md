...
2. Open the implementation file the test targets. Introduce exactly ONE
   ...
   new test is meant to cover:
   - Flip a comparison operator (`>` to `>=`, `==` to `!=`, `&&` to `||`).
   - Change a returned literal or default value (`return true;` to
     `return false;`, `0` to `1`).
...
the `mutation_test` package for Dart — outside anything this skill
generates. Only bring this up if the user asks for something more
rigorous than this check.
